...
var ejs = require("ejs");
var common = require("./common.js");//마치 라이브러리를 가져오듯 사용자 정의 모듈을 가지고 온다.
...
app.post("/notice/update", function(request, response){
    ...
    var sql = "update notice set title = '"+title+"', writer = '"+writer+"', content = '"+content+"' where notice_id="+notice_id;
    ...
    con.query(sql, function(error, fields){
        if(error){
            ...
            //메시지 출력후, list 요청
            response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
            response.end(common.getMsgURL("수정성공", "/notice/detail?notice_id="+notice_id));
        }
    });
});
